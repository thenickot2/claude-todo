...
├── capabilities/default.json  # Tauri permission scopes (fs, shell, watch)
...
- **Storage**: Single markdown file at `~/.claude-todo/todos.md`. Three sections (`## Not Started`, `## In Progress`, `## Done`). Metadata is pipe-delimited: `- [ ] Title | key:value | key:value`.
...
Scoped in `src-tauri/capabilities/default.json`. Each fs/shell operation needs an explicit permission entry with allowed paths. Shell commands (claude, wt) are scoped in `tauri.conf.json` under `plugins.shell.scope`.
...
- **CSS** — single `styles.css` file, CSS custom properties for theming, class naming is flat (`.todo-item`, `.btn-sm`).
- **Tests** — vitest for TypeScript. Test files live next to source (`storage.test.ts`). Run with `npx vitest run`.

## Current State

Phases 0–4 complete (setup, storage, UI, Claude integration, Windows window management). See `PLAN.md` for full progress tracker. Remaining: Phase 5 (macOS), Phase 6 (polish/ship).
